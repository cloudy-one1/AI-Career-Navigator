...
> 2. **版本迭代叙事** → [CHANGELOG.md](CHANGELOG.md)（v2 → v8.3 各轮新增/推翻/修复）
...
- **当前版本**：v8.3.3（v8.4「能力画像按岗位分组」在研，设计稿见 `docs/specs/`，见 CHANGELOG.md）
...
python -m pytest tests/ -q       # 运行后端测试套件（当前 1039 用例 + 1 live_llm 抽检默认跳过）
...
│                           #   + state 单例 + deps 依赖）
├── frontend/               # 原生 ES Module SPA（Vite）+ Chart.js（双入口：index + landing）
│   ├── index.html                                         # 主 SPA 入口（v7.3 旅程分组导航；v7.5 删分享页入口）
│   ├── landing.html                                       # [v8.2] 独立产品落地页（后端 `/` 返回）
...
├── tests/                  # pytest 测试套件（1039 用例 + 1 live_llm 抽检；含 golden 样本回归、WS 主循环集成）
