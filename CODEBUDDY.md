...
- **定位（v7.3，v7.5 收缩）**：全流程求职陪跑平台「AI 求职陪跑」——五步旅程：定方向（市场数据/岗位库）→ 备弹药（简历库）→ 演练（模拟面试/题库/历史）→ 诊弱点（综合报告/长期记忆）→ 定规划（职业规划）。导航按旅程分组（备战/演练/洞察 + 账户）；v7.5 删除招聘者端与报告分享（"连机会"取消，回归求职者单端，见 CHARTER DC-08）；决策记录见 CHARTER DC-07，方案全文见 `docs/产品定位延伸_全流程求职陪跑.md`。
...
python -m pytest tests/ -q       # 运行后端测试套件（当前 996 用例 + 1 live_llm 抽检默认跳过）
...
├── tests/                  # pytest 测试套件（996 用例 + 1 live_llm 抽检；含 golden 样本回归、WS 主循环集成）
