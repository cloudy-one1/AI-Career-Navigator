...
<p align="center">
  <strong>v7.4.0 — 全流程求职陪跑平台（v7.3 定位延伸；本轮：语音链路强化——长录音 413 / 熔断过脆 / 免手闭环 + 前端测试从零起步）</strong>
</p>
...
- **自动化测试**：后端 1028 个 pytest 用例覆盖核心路径（含依赖 API Key 的 LLM 类测试；v7.0.3 起新增黄金样本回归，v7.2.1 扩容至 20 条并接入 CI）；v7.4 起前端新增 vitest 套件，首批 16 例钉住 `voice.js` 的世代号竞态、熔断韧性与 VAD 状态机（此前这三个模块零自动化覆盖）
...
│
├── tests/                        # 后端自动化测试（1028 用例 + 1 live_llm 抽检，本机非 LLM 类全绿）
...
| **测试** | pytest（后端 1028 用例 + 1 live_llm 抽检）+ vitest（前端 16 例，v7.4 起，本机非 LLM 类全绿） |
...
本项目的测试不是「堆数量」，而是**分层保障 + 评测（eval）**两套机制配合。1028 个自动化用例全绿，结构如下：
...
| **参考答案背诵面板未渲染** | `detailed_qa`（逐题改写答案/背诵材料）目前仅进 Markdown 复盘导出（`generate_review_markdown`）；报告 Tab 自 v6.2 起已渲染 `qa_breakdown` 逐题拆解（含「✍️ 含改写示范」徽标但不展示答案正文），字段已与 `detailed_qa` 对齐；分享页刻意不输出改后答案（学习材料留给候选人本人） | 在报告 Tab 补逐题参考答案背诵面板（可直接复用 `qa_breakdown` 同一套渲染） |
...
```bash
# 运行全部测试（1028 用例，全绿；push/PR 由 GitHub Actions 自动执行）
pytest tests/ -v
