...
<p align="center">
  <strong>v7.3.1 — 全流程求职陪跑平台（v7.3 定位延伸；本轮：品牌/版本统一 + 面试主循环测试补位）</strong>
</p>
...
- **自动化测试**：1028 个测试用例覆盖核心路径（含依赖 API Key 的 LLM 类测试；v7.0.3 起新增黄金样本回归，v7.2.1 扩容至 20 条并接入 CI）
...
│
├── tests/                        # 自动化测试（1028 用例 + 1 live_llm 抽检，本机非 LLM 类全绿）
...
| **测试** | pytest（1028 用例 + 1 live_llm 抽检，本机非 LLM 类全绿） |
