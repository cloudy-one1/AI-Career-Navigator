...
<p align="center">
  <strong>v7.5.0 — 全流程求职陪跑平台（v7.3 定位延伸；本轮：范围收敛——删除招聘者端与报告分享，回归求职者单端）</strong>
</p>
...
AI 求职陪跑平台是一个面向求职者的全流程 AI 陪跑系统，按五步主线旅程组织（v7.3 定位延伸，功能零增删；v7.5 范围收缩取消"连机会"）：

**定方向**（市场数据 + 岗位库）→ **备弹药**（简历库）→ **演练**（AI 多轮模拟面试：6 阶段拟真模式 / 5 轮次传统模式）→ **诊弱点**（围绕 **STAR 完整性、量化程度、逻辑连贯性、岗位相关性、专业深度** 五个维度实时诊断，生成改写建议与综合评分报告 + 长期记忆图谱）→ **定规划**（时间轴职业路径）。
...
- **自动化测试**：后端 996 个 pytest 用例覆盖核心路径（含依赖 API Key 的 LLM 类测试；v7.0.3 起新增黄金样本回归，v7.2.1 扩容至 20 条并接入 CI）；v7.4 起前端新增 vitest 套件，首批 16 例钉住 `voice.js` 的世代号竞态、熔断韧性与 VAD 状态机（此前这三个模块零自动化覆盖）
...
│
├── tests/                        # 后端自动化测试（996 用例 + 1 live_llm 抽检，本机非 LLM 类全绿）
...
| **测试** | pytest（后端 996 用例 + 1 live_llm 抽检）+ vitest（前端 16 例，v7.4 起，本机非 LLM 类全绿） |
...
系统**只服务求职者**——v7.0 曾引入"招聘者只读端 + 报告分享"，因与"面向求职者"的定位冲突（画蛇添足）已在本版删除（CHARTER **DC-08**）。当前只有一套登录、一个视图：

1. **注册/登录**（可选）：顶部「账户」面板注册，无需选择身份。不登录也能完整使用——登录后简历库/岗位库/历史记录跨设备归集到你的账户下。
...
本项目的测试不是「堆数量」，而是**分层保障 + 评测（eval）**两套机制配合。996 个自动化用例全绿，结构如下：
...
```bash
# 运行全部测试（1033 用例，全绿；push/PR 由 GitHub Actions 自动执行）
pytest tests/ -v
