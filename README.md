...
<p align="center">
  <strong>v8.1.0 — 全流程求职陪跑平台（v8.0 引入求职档案领域核心；本轮：全站术语统一为专业评测语系 + 能力成长曲线 / 五步完成度 / 技能缺口）</strong>
</p>
...
AI 求职陪跑平台是一个面向求职者的全流程 AI 陪跑系统，按五步主线组织（v7.3 定位延伸，功能零增删；v7.5 范围收缩取消"连机会"；**v8.1 术语统一为专业评测语系，步骤改名、结构不变**，决策记录见 CHARTER **DC-09**）：
...
- **术语统一为专业评测语系（v8.1）**：「作战室」→ **能力档案**，五步主线改为 **职业定位 / 简历准备 / 面试演练 / 能力诊断 / 发展路径**，「下一步最佳动作」→ **下一步建议**，「能力雷达」→ **能力画像**——游戏化与军事隐喻和「专业评测工具」定位冲突，削弱诊断数据可信度；**只改显示名，tab key 与数据结构零变更**（决策记录见 CHARTER **DC-09**）。同期补三项能力：**能力成长曲线**（一次 JOIN 取数替代逐份 `get_report` 的 N+1，少于两个点不画线而给空态引导）、**五步完成度三态**（能推导就不落库，仅无法推导的「发展路径」走 `journey_marks` 打点，未登录不落库）、**技能缺口**（简历技能 vs 市场热门技能，集合运算而非 LLM，确定性事实判断不该由模型猜）
- **产品定位延伸为「全流程求职陪跑平台」（v7.3）**：15 个功能域全部归入五步旅程、零增删——侧边栏按旅程重组为 备战 / 演练 / 洞察 + 账户（桌面与移动端同步重排，默认落地页仍为模拟面试），品牌统一为「AI 求职陪跑」（主界面 / 导出页 / FastAPI title）；决策记录见 CHARTER **DC-07**；v7.5 范围收缩：删除招聘者端与报告分享（"连机会"取消，六步改五步），回归求职者单端，见 **DC-08**
...
- **侧边栏信息架构分组（v7.2）**：11 个入口平铺 → 面试域 / 资产域 / 洞察域 + 账户分区（v7.5 删除"招聘端"分组与按身份显隐联动）
...
- **认证与资源归属（v7.0）**：轻量认证（bcrypt + JWT，可关闭开关 `AUTH_ENABLED=false` 回退旧行为）+ 资源归属（会话/简历/岗位按 owner 隔离，WebSocket 握手校验身份）+ **简历库/岗位库**（跨会话复用，不必重复上传与解析）；v7.0 同期引入的"报告分享链接 + 招聘者只读端"已于 v7.5 删除（回归求职者单端，见 CHARTER **DC-08**）
...
│       │   ├── navConfig.js      # [v8.0 NEW] 导航单一数据源（五步旅程 + 账户，侧栏与底部导航同源于此）
...
│   ├── test_profile_api.py          # [v8.0 NEW] /api/profile 与 /api/profile/refresh（owner 隔离 + 缓存失效）
...
系统**只服务求职者**——v7.0 曾引入"招聘者只读端 + 报告分享"，因与"面向求职者"的定位冲突（画蛇添足）已于 v7.5 删除（CHARTER **DC-08**）。当前只有一套登录、一个视图：
...
2. **注册/登录**（可选）：顶部「账户」面板注册，无需选择身份。不登录也能完整使用——登录后简历库/岗位库/历史记录跨设备归集到你的账户下。
3. **建立素材库**：「简历库」上传一次简历（完整入库，不截断）；「岗位库」保存常练的岗位 JD。之后每次开练直接选用，不必重复上传与解析。
4. **开练**：面试面板选择「从简历库选择 / 粘贴上传」与「从岗位库选择 / 粘贴 JD」→ 开始面试。
5. **复盘**：报告页的每维度评分附**候选人原话引用**（quote），可逐条核对"这个分凭什么"；复盘成果支持 Markdown / HTML 导出自用。
...
本项目的测试不是「堆数量」，而是**分层保障 + 评测（eval）**两套机制配合。1026 个自动化用例全绿，结构如下：
...
| ③ 安全与边界 | test_security / test_auth | 注入拦截、越权、恢复红线 | 课程项目级护栏的可验证证据 |
...
| **语音端点无鉴权（v7.4 已知）** | `/api/voice/tts`、`/api/voice/asr` 未挂鉴权依赖，`AUTH_ENABLED=true` 时仍公开，匿名可调；唯一防线是 `RATE_LIMIT_VOICE=20/min`（按 IP 计数） | 给 voice 路由挂 `deps` 鉴权；按用户维度限流与计费配额 |
...
| **档案「待提升项」沿用全局薄弱点（v8.0 登记）** | `weakness_memory` 以 dimension 为主键、**全局无 owner 维度**（v6.3 早于 v7.0 认证），因此档案的待提升项本阶段未做用户隔离——不同登录用户的薄弱点会互相可见 | 按 `db._ensure_owner_columns` 的 PRAGMA + ALTER 范式给 weakness_memory 补 `owner_id`，档案侧随之按 owner 过滤 |
